...
func TestGameMultiround(t *testing.T) {
	...
	// Pre flop
	assert.Equal(t, string(RoundPreFlop), game.GetPlayerState(&simon).BettingRound)
	err = game.TakeAction(&jason, Action{ActionType: ActionTypeBet, Value: 8})
	assert.NoError(t, err)
	...
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 1000})
	assert.Error(t, err)
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 10})
	assert.NoError(t, err)
	...
	// Can't play out of turn
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 5})
	assert.Error(t, err)
	err = game.TakeAction(&chien, Action{ActionType: ActionTypeBet, Value: 5})
	assert.Error(t, err)
	err = game.TakeAction(&jarry, Action{ActionType: ActionTypeBet, Value: 5})
	...
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 5})
	assert.Error(t, err)
	err = game.TakeAction(&jason, Action{ActionType: ActionTypeBet, Value: 5})
	assert.Error(t, err)
	...
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 10})
	assert.NoError(t, err)
	err = game.TakeAction(&jarry, Action{ActionType: ActionTypeFold})
	assert.NoError(t, err)
	...
	assert.Equal(t, string(RoundFlop), game.GetPlayerState(&simon).BettingRound)
	// DONK!
	err = game.TakeAction(&jarry, Action{ActionType: ActionTypeBet, Value: 50})
	assert.NoError(t, err)
	assert.Equal(t, int32(26), game.GetPlayerState(&jarry).Seats[jarry.SeatNumber].Player.Stack)
	err = game.TakeAction(&jason, Action{ActionType: ActionTypeFold})
	assert.NoError(t, err)
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeFold})
	assert.NoError(t, err)
	err = game.TakeAction(&chien, Action{ActionType: ActionTypeFold})
	assert.NoError(t, err)
...
func TestGameAllInSimple(t *testing.T) {
	...
	assert.Equal(t, string(RoundFlop), game.GetPlayerState(&simon).BettingRound)
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 30})
	assert.NoError(t, err)
...
func TestGameAllInTwoSidePots(t *testing.T) {
	...
	assert.Equal(t, string(RoundPreFlop), game.GetPlayerState(&simon).BettingRound)
	err = game.TakeAction(&jarry, Action{ActionType: ActionTypeBet, Value: 8})
	assert.NoError(t, err)
	...
	assert.Equal(t, string(RoundFlop), game.GetPlayerState(&simon).BettingRound)
	err = game.TakeAction(&simon, Action{ActionType: ActionTypeBet, Value: 15})
	assert.NoError(t, err)
