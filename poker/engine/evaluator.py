...
def straight_flush_check(hand):
    ...
    frequencies = [suit for suit in set(suits) if suits.count(suit) >= 5] 
...
def quad_check(hand):
    ...
    frequencies = [rank_id for rank_id in set(values) if values.count(rank_id) == 4]
...
def boat_check(hand):
    values = [card.rank for card in hand]
    trips = [rank_id for rank_id in set(values) if values.count(rank_id) >= 3]
    if not trips:
        return False, None
    top_trip = max(trips)
    pairs = [rank_id for rank_id in set(values) if values.count(rank_id) >= 2 and rank_id != top_trip]
    if not pairs:
        return False, None
    top_pair = max(pairs)
...
def trip_check(hand):
    values = [card.rank for card in hand]
    frequencies = [rank_id for rank_id in set(values) if values.count(rank_id) >= 3]
    hit = len(frequencies) >= 1
...
def two_pair_check(hand):
    values = [card.rank for card in hand]
    frequencies = sorted([rank_id for rank_id in set(values) if values.count(rank_id) == 2], lambda x: -x)
